using System;
using System.Collections.Generic;
using System.ComponentModel.Composition;
using System.IO;
using System.Linq;
using System.Reflection;
using XrmToolBox.Extensibility;
using XrmToolBox.Extensibility.Interfaces;

namespace GM.XrmToolBox.UniversalTranslationManager
{
    [Export(typeof(IXrmToolBoxPlugin)),
        ExportMetadata("Name", "Universal Translation Manager"),
        ExportMetadata("Description", "UI-only hybrid manager for Dataverse and Dynamics 365 metadata translations"),
        ExportMetadata("SmallImageBase64", ""),
        ExportMetadata("BigImageBase64", ""),
        ExportMetadata("BackgroundColor", "White"),
        ExportMetadata("PrimaryFontColor", "Black"),
        ExportMetadata("SecondaryFontColor", "Gray")]
    public sealed class MyPlugin : PluginBase
    {
        public override IXrmToolBoxPluginControl GetControl()
        {
            return new MyPluginControl();
        }

        public MyPlugin()
        {
            // Uncomment the next line if the plugin needs to resolve dependencies from a subfolder.
            // AppDomain.CurrentDomain.AssemblyResolve += new ResolveEventHandler(AssemblyResolveEventHandler);
        }

        private Assembly AssemblyResolveEventHandler(object sender, ResolveEventArgs args)
        {
            Assembly loadAssembly = null;
            Assembly currentAssembly = Assembly.GetExecutingAssembly();

            string argumentName = args.Name.Substring(0, args.Name.IndexOf(",", StringComparison.Ordinal));
            List<AssemblyName> referencedAssemblies = currentAssembly.GetReferencedAssemblies().ToList();
            AssemblyName referencedAssembly = referencedAssemblies.FirstOrDefault(a => a.Name == argumentName);

            if (referencedAssembly != null)
            {
                string folder = Path.GetDirectoryName(currentAssembly.Location);
                string assemblyFolder = Path.GetFileNameWithoutExtension(currentAssembly.Location);
                string dependencyFolder = Path.Combine(folder ?? string.Empty, assemblyFolder);
                string dependencyPath = Path.Combine(dependencyFolder, argumentName + ".dll");

                if (File.Exists(dependencyPath))
                {
                    loadAssembly = Assembly.LoadFrom(dependencyPath);
                }
                else
                {
                    throw new FileNotFoundException("Unable to locate dependency: " + dependencyPath);
                }
            }

            return loadAssembly;
        }
    }
}
